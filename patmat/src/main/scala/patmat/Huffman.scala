...
object Huffman {
   ...
   *
   * Here a represents a left branch of a Fork and b the right branch, so we assume the lists of
   * characters of both sides are disjoint and the resulting CodeTable corresponds to
   * Fork(a, b, chars(a) ::: chars(b), weight(a) + weight(b))
   */
    /* // Case with if/else
    ...
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = List(a, b) match {
